...
static class Placement
{
    ...
    {
        int tileWidth = image.width / blockWidth;
        int tileHeight = image.height / blockHeight;
        bool[,] tile = new bool[tileHeight,tileWidth];
        Color currentPixel;
        for (int i = 0; i < tileHeight; ++i)
        {
            for (int j = 0; j < tileWidth; ++j)
            {
                currentPixel = image.GetPixel(j * tileWidth + tileWidth / 2, i * tileHeight + tileHeight / 2);
                tile[j,i] = (specialColor == currentPixel) ? isSpecialColorTrue : !isSpecialColorTrue;
            }
        }
        return tile;
    }
